...
function addLine(x1, y1, x2, y2) {
  ...
  lines.set(`${kx1}_${ky1}_${kx2}_${ky2}`, [x1, y1, x2, y2, randColor(), Math.random()]);
}
...
function initializeLine(xStart, yStart, xFinish, yFinish) {
  ...
  if (Math.random() > lineSparseness) { // 0 to 1
    addLine(
      xStart + xShift,
      yStart + yShift,
...
function initializeCanvas() {
  ...
  for (let y = 0; y < cHeight; y += ystep) {
    for (let xraw = 0; xraw < cWidth; xraw += xstep * 3) {
      ...
      initializeLine(x + xstep * 3, y + ystep * 2, x + xstep * 2, y + ystep * 4)
...
function render() {
  ...
  lines.forEach(coords => {
    ...
    let progressPercent = (Math.max(now - start - delayMs, 0) / duration) % cycleLength
    ...
    context.lineTo(x1 + (x2 - x1) * progressPercent, y1 + (y2 - y1) * progressPercent);
...
function handleAmbition() {
  ...
  fetch('/endpoint?r=WW91J3JlIGxvb2tpbmcgaW4gdGhlIHdyb25nIHBsYWNl')
    .then(handleResponse)
}